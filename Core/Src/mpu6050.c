...
void MPU6050_ReadAll(MPU6050_Data *data) {
    ...
    data->temp = raw_temp / 340.0f + 36.53f - 25.0f;

    /* ================= 互补滤波 (标准 一阶互补) ================= */
    /* angle = α*(angle + gyro*dt) + (1-α)*accel_angle               */
    /* α=0.98: 陀螺仪权重 98%, 加速度计权重 2%                        */
    ...
    /* 3. 互补滤波系数: 静止时 α=0.98, 运动时纯陀螺仪 α=1.0 */
    float alpha = (accel_mag > 0.9f && accel_mag < 1.1f) ? 0.98f : 1.0f;

    /* 4. 加速度计角度 */
    /* pitch: 绕 Y 轴旋转 → 用 accel_x / sqrt(accel_y²+accel_z²) */
    float accel_pitch = atan2f(data->accel_x,
        sqrtf(data->accel_y * data->accel_y + data->accel_z * data->accel_z)) * 57.2958f;
    /* roll:  绕 X 轴旋转 → 用 accel_y / accel_z */
    float accel_roll  = atan2f(-data->accel_y, data->accel_z) * 57.2958f;

    /* 5. 互补滤波: pitch 对应 Y 轴旋转 → gyro_y */
    gyro_angle_x = alpha * (gyro_angle_x + data->gyro_y * dt)
                 + (1.0f - alpha) * accel_pitch;
    /* roll 对应 X 轴旋转 → gyro_x */
    gyro_angle_y = alpha * (gyro_angle_y + data->gyro_x * dt)
...
void MPU6050_StepProcess(MPU6050_Data *data) {
    ...
    if (sig > last_sig) {
        ...
        if (last_sig > dynamic_threshold && 
            ...
            data->step_count++;
            data->calories = (float)data->step_count * 0.04f;
            data->distance = (float)data->step_count * 0.00075f;
            last_step_time = now;
