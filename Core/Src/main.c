...
/* 息屏控制: 0=亮屏, 1=息屏; 上次用户操作时刻; 时钟页面进入时刻 */
uint8_t g_screen_off = 0;
TickType_t g_last_activity_tick = 0;
TickType_t g_screen_dim_tick = 0;

/* Setting 子菜单 & 亮度 (0x00~0xFF, 默认最亮) */
uint8_t g_setting_index = 0;
uint8_t g_brightness = 0xFF;

/* 手电筒: 子菜单索引, 点亮状态 */
uint8_t g_flashlight_index = 0;
uint8_t g_flashlight_on = 0;

/* 电池电压 (mV, 通过 VREFINT 反算) */
...
static void MX_ADC1_Init(void)
{
    ...
    /* ADC 时钟: APB2=72MHz → /6 = 12MHz (<14MHz 上限) */
    __HAL_RCC_ADC_CONFIG(RCC_ADCPCLK2_DIV6);

    /* 先使能 VREFINT, 再让 HAL_ADC_Init 上电 (一同稳定) */
    ...
    /* TSVREFE 可能被 HAL_ADC_Init 复位, 重新使能 */
    ADC1->CR2 |= ADC_CR2_TSVREFE;
...
/* ======== 读取供电电压 (mV), 通过 VREFINT 反算 VDD ======== */
/* 原理: VREFINT 恒定约 1.20V, ADC 以 VDD 为参考,           */
/* 工厂校准值存在 0x1FFFF7BA, 公式 VDD_mV = 3300*CAL/raw   */
...
uint16_t Battery_ReadMV(void)
{
    ...
    int32_t result = (int32_t)(3300UL * vrefint_cal / adc_raw) - 14700;
    return (result > 0) ? (uint16_t)result : 0;
}

/* ADC 注入通道转换完成回调 (HAL 弱定义, 需提供实体) */
...
void vTask_Sensor(void const * argument)
{
  ...
  for(;;)
  {
    ...
    g_sensor.calories   = mpu_data.calories;
	    g_sensor.distance   = mpu_data.distance;

    /* 电池电压: 每 5 次循环 (1 秒) 读取一次 */
    {
      static uint8_t bat_cnt = 0;
      bat_cnt++;
      if (bat_cnt >= 5) {
        bat_cnt = 0;
        g_battery_mv = Battery_ReadMV();
      }
    }
...
void vTask_Display(void const * argument)
{
  ...
  char buf[22];
  const char *menu_labels[] = {"Steps", "Sensor", "Clock", "Flashlight", "Setting"};
  ...
  for(;;)
  {
    ...
    case PAGE_CLOCK: {
      ...
      /* 大字时间: 2x3 缩放, HH:MM:SS, x=11 y=20 */
      {
        char time_buf[9];
        ...
        for (uint8_t i = 0; time_buf[i]; i++) {
          if (time_buf[i] == ':') {
            SSD1306_DrawBigChar(':', x_pos, 20, 1, 3);
            x_pos += 6;
          } else {
            SSD1306_DrawBigChar(time_buf[i], x_pos, 20, 2, 3);
            x_pos += 12;
          }
        }
      }
    ...
    case PAGE_STEPS:
      ...
      SSD1306_WriteString(buf);
      SSD1306_SetCursor(0, 4);
      snprintf(buf, sizeof(buf), "Cal: %.1f kcal",
               g_sensor.calories);
      SSD1306_WriteString(buf);
      SSD1306_SetCursor(0, 5);
      snprintf(buf, sizeof(buf), "Dist: %.3f km",
               g_sensor.distance);
      SSD1306_WriteString(buf);
    ...
    case PAGE_SENSOR:
      ...
      SSD1306_WriteString(buf);
      SSD1306_SetCursor(0, 5);
      snprintf(buf, sizeof(buf), "Bat: %u.%02uV",
    ...
    case PAGE_SETTING: {
      const char *setting_items[] = {"Light", "Bluetooth", "About", "Back"};
      SSD1306_SetCursor(0, 0);
      SSD1306_WriteString("== Setting ==");
      for (uint8_t i = 0; i < SETTING_ITEMS; i++) {
        SSD1306_SetCursor(0, 3 + i);
        SSD1306_WriteString(i == g_setting_index ? ">" : " ");
        SSD1306_WriteString((char *)setting_items[i]);
      }
      break;
    }
    ...
    case PAGE_LIGHT: {
      ...
      /* 亮度百分比 */
      uint8_t pct = (uint8_t)((uint16_t)g_brightness * 100 / 255);
      SSD1306_SetCursor(10, 2);
      snprintf(buf, sizeof(buf), "%u%%", pct);
      SSD1306_WriteString(buf);
      /* 亮度条: 128px → 映射 0~255 */
      {
        uint8_t bar_w = (uint8_t)((uint16_t)g_brightness * 120 / 255);
        SSD1306_DrawRect(4, 30, 120, 12);
        if (bar_w > 0) SSD1306_FillRect(5, 31, bar_w, 10);
      }
      SSD1306_SetCursor(0, 6);
      SSD1306_WriteString("Rotate +-  Press OK");
      break;
    ...
    case PAGE_FLASHLIGHT: {
      if (g_flashlight_on) {
        ...
        const char *flashlight_items[] = {"Light", "Back"};
        SSD1306_SetCursor(0, 0);
        SSD1306_WriteString("== Flashlight ==");
        for (uint8_t i = 0; i < 2; i++) {
          SSD1306_SetCursor(0, 3 + i);
          SSD1306_WriteString(i == g_flashlight_index ? ">" : " ");
          SSD1306_WriteString((char *)flashlight_items[i]);
        }
      }
      break;
    }
...
void vTask_Menu(void const * argument)
{
  ...
  for(;;)
  {
    ...
    btn_pressed = Encoder_ButtonPressed();

    /* ==== 追踪用户活动 ==== */
    if (delta != 0 || btn_pressed) {
      g_last_activity_tick = xTaskGetTickCount();
      g_screen_dim_tick   = xTaskGetTickCount();
    }
    ...
    /* ==== Setting 子菜单: 旋转选项目 ==== */
    if (delta != 0 && g_current_page == PAGE_SETTING) {
      if (delta > 0 && g_setting_index < SETTING_ITEMS - 1) g_setting_index++;
      else if (delta < 0 && g_setting_index > 0)             g_setting_index--;
      osSemaphoreRelease(semDisplayUpdateHandle);
    }

    /* ==== Light 页面: 旋转调节亮度 ==== */
    if (delta != 0 && g_current_page == PAGE_LIGHT) {
      if (delta > 0 && g_brightness < 246) g_brightness += 10;
      else if (delta < 0 && g_brightness > 10) g_brightness -= 10;
      osMutexWait(mutexI2CHandle, osWaitForever);
    ...
    /* ==== Flashlight 子菜单: 旋转选项目 ==== */
    if (delta != 0 && g_current_page == PAGE_FLASHLIGHT && !g_flashlight_on) {
      if (delta > 0 && g_flashlight_index < 1) g_flashlight_index++;
      else if (delta < 0 && g_flashlight_index > 0) g_flashlight_index--;
      osSemaphoreRelease(semDisplayUpdateHandle);
    }
    ...
    if (btn_pressed) {
      ...
      case PAGE_MENU:
        ...
        case 2:
          ...
          break;
        case 3:
          g_current_page = PAGE_FLASHLIGHT;
          g_flashlight_index = 0;
          g_flashlight_on = 0;
          break;
        case 4:
      ...
      case PAGE_SETTING:
        ...
        case 0: /* Light */
          g_current_page = PAGE_LIGHT;
          break;
        case 1: /* Bluetooth */
          g_current_page = PAGE_BLUETOOTH;
          g_bt_btn_idx = 0;
          break;
        case 2: /* About */
          g_current_page = PAGE_ABOUT;
          break;
        case 3: /* Back */
          g_current_page = PAGE_MENU;
          menu_activity_tick = xTaskGetTickCount();
    ...
    /* ==== 息屏: 时钟页面 7 秒无操作 → 关闭 OLED ==== */
    if (g_current_page == PAGE_CLOCK) {
      if ((xTaskGetTickCount() - g_screen_dim_tick) >= pdMS_TO_TICKS(7000)) {
        g_screen_off = 1;
